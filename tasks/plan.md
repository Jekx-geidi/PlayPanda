...
- [x] Task 5: Registration page (standard user flow, PLAYPANDA_REGISTER_PAGE.md MVP scope + user request)
  - Initial version was Google-only (matching the spec's wireframe). User explicitly asked for `/register` to be "a fill up form" — clarified via AskUserQuestion to mean: a real manual signup form (Full Name, Email, Password, Confirm Password, Display Name, Contact Number, User Type), with Google kept alongside as a faster alternative, not a replacement.
  - `/register` now has both: the form (`AuthContext.signUpWithEmail` → `supabase.auth.signUp` with profile fields carried in `options.data` user_metadata, since email-confirmation can delay the session by an arbitrary amount and metadata survives that gap) and "Continue with Google" (unchanged OAuth flow, redirects back to `/register`).
  - Post-auth resolution (fires whenever `session` becomes non-null, from either path): `profiles` row → straight to Admin/Scorer dashboard; existing `user_accounts` row → straight to `/` (REG section 12, no duplicate); brand-new session whose `user_metadata` already has `display_name`/`user_type` (i.e. came from the form) → creates the `user_accounts` row right then and redirects to `/`; brand-new session with no such metadata (i.e. came from Google, which doesn't know Display Name/User Type/Terms) → sent to `/register/profile` to finish those three fields. If sign-up needs email confirmation (`data.session` is null), shows an inline "Almost There! We sent a confirmation link to `<email>`" state instead of navigating anywhere.
  - `/register/profile` gained matching Full Name + Contact Number fields (previously just Display Name + User Type) for parity between the two entry paths — same `user_accounts` row either way.
  - New `supabase/migrations/0002_create_user_accounts.sql` (still unapplied, safe to edit directly): `full_name`, `display_name`, `user_type`, `contact_number` (nullable), `terms_accepted_at`. Deliberately a separate table from `profiles` — `user_type` is display/business metadata only and carries no authorization meaning (REG section 35), unlike `profiles.role`. RLS: select/insert/update own row only, `WITH CHECK (auth.uid() = id)` on both insert and update.
  - `src/lib/account.ts` (new): `getUserAccount`/`createUserAccount`. `src/lib/userTypes.ts` (new): shared `UserType`/`USER_TYPES` used by both pages.
  - `AuthContext.signInWithGoogle` now takes an optional `redirectPath` (defaults to `/login`). New `AuthContext.signUpWithEmail`. Login and Register cross-link.
  - Header's "Live Scores" pill replaced with a "Register" link (user request) — no live-scoring feature exists yet to justify that pill.
  - 22 unit tests total (mocked Supabase, table-aware `.from()` mock, and a captured `onAuthStateChange` callback manually fired in tests to simulate Supabase's real post-`signUp` `SIGNED_IN` event). Build clean. Live-verified in browser: full form renders per the fill-up-form request, inline validation fires correctly (empty submit, mismatched passwords), and `/register/profile` + Google-consent-screen wiring still both work.
  - **Not yet live-verified** (needs a real signup, which needs SMTP or "confirm email" disabled in the dashboard): the actual email-confirmation round trip — does Supabase really redirect back to `/register` with a working session afterward. Confirmed correct by code/test only.
